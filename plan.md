...
在系统跑通 MVP 并稳定运行后，为解决潜在的边界情况和格式问题，计划引入以下优化策略：

### 1. 结构化输出的稳定性增强
- **痛点**：要求 LLM 直接输出带有特定格式的 Markdown 和 YAML frontmatter 时，常因微小的格式错误（如多余空格、缺失引号）导致后续插件解析失败。
- **优化方案**：
  - 强制 LLM 在编译时以 JSON 格式输出（或使用 Function Calling 约束结构）。
  - 在获取到标准的 JSON 数据后，由后端 Node.js 代码通过模板引擎（如 Handlebars 或原生字符串模板）来组装并写入最终的 Markdown 文本，确保输出格式 100% 符合规范。

### 2. 异常处理机制 (死信队列)
- **痛点**：若因网络抖动、文件解析失败或 API 超时导致某篇文章编译失败，需要有明确的机制进行兜底。
- **优化方案**：
  - 引入异常处理机制，编译失败的文件将自动被打上 `#status/error` 标签，或移至专门的 `11-Error/` 目录。
  - 在文件头部或同级目录自动追加 Error Log，方便人工介入排查与重试。

### 3. 多端同步冲突处理
- **痛点**：AI 编译服务在 Ubuntu 服务器端异步写入文件，而用户可能在本地 Obsidian 中同时查阅修改，易引发覆盖或版本冲突。
- **优化方案**：
  - 确立状态锁机制：用户在本地人工修改或审阅后的文件需打上 `#status/reviewed` 标签，后端系统扫描到该标签后，AI 引擎将不再对该文件进行覆盖和更新。
  - 定期利用脚本进行 Git 自动 Commit + Push，或配合 Syncthing 等双向同步工具处理可能存在的多端文本合并冲突。
